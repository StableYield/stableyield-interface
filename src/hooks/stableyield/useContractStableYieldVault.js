...
import StableYieldVault_ABI from "../../../contracts/stableyield/StableYieldVault.json";
...
/**
 * @name useContractStableYieldVault
 * @param {*} addressContract
 */
export function useContractStableYieldVault(address) {
  const { account } = useWeb3React();
  const [contract, isDeployed, isSigner] = useContractDeployedAndSigner(
    address,
    StableYieldVault_ABI
  );
  ...
  return {
    ...
    swapCollateral: useContractTransactionLifecycle(
      idx(contract, (_) => _.swapCollateral)
    ),
  };
}
