...
/**
 * @name FormStableYieldBorrowCredit
 * @description Deposit Collateral into the Aave LendingPool
 * @version 0.0.1
 */

export const FormStableYieldBorrowCredit = ({
  address,
  defaultCollateral,
  ...props
}) => {
  ...
  const { borrowCredit } = useContractStableYieldVaultWithCreditDelegation(
  ...
  return (
    <>
      ...
      {borrowCredit && (
        <TransactionModal
          isConnected={borrowCredit.method}
          transaction={borrowCredit}
        />
      )}
    </>
  );
...
FormStableYieldBorrowCredit.defaultProps = {
  label: "Withdraw",
};

FormStableYieldBorrowCredit.propTypes = {
  label: PropTypes.string,
};

export default FormStableYieldBorrowCredit;
